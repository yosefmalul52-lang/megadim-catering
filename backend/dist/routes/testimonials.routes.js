...
const loadTestimonials = async () => {
    try {
        ...
        const data = await promises_1.default.readFile(testimonialsPath, 'utf-8');
...
const saveTestimonials = async (testimonials) => {
    try {
        const testimonialsPath = path_1.default.join(__dirname, '..', 'data', 'testimonials.json');
        await promises_1.default.writeFile(testimonialsPath, JSON.stringify(testimonials, null, 2));
    }
    catch (error) {
...
router.get('/admin/all', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const testimonials = await loadTestimonials();
    res.status(200).json({
        success: true,
        data: testimonials,
